"use client";

import { useState } from "react";
import { useDemoData } from "@/lib/demoData";
import Artboard from "@/components/Artboard";
import SiteFooter from "@/components/SiteFooter";
import Link from "next/link";
import { BoardView } from "@/hooks/useForceGraph";

/**
 * Static demo board — read-only snapshot of the live board served from
 * local JSON files under /demo-data (generated by `npm run export-demo`).
 * No Firebase/Firestore requests happen here.
 */
export default function DemoHome() {
  const { nodes, submissions, loading, error, exportedAt } = useDemoData();
  const [view, setView] = useState<BoardView>("grid");

  if (loading) {
    return (
      <div className="page-shell flex min-h-screen items-center justify-center">
        <div className="surface-card-soft px-8 py-10 text-center">
          <div className="mx-auto mb-4 h-14 w-14 animate-spin rounded-full border-2 border-cyan-400/30 border-t-cyan-400"></div>
          <p className="text-slate-300">Loading demo data...</p>
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="page-shell flex min-h-screen items-center justify-center p-4">
        <div className="surface-card-soft max-w-md px-8 py-8 text-center">
          <p className="mb-2 text-lg font-semibold text-white">
            Demo data unavailable
          </p>
          <p className="mb-4 text-sm text-slate-400">
            Run <code className="text-cyan-300">npm run export-demo</code> to
            generate the snapshot under <code className="text-cyan-300">public/demo-data/</code>.
          </p>
          <Link href="/" className="primary-btn px-4 py-2">
            Back to Live Board
          </Link>
        </div>
      </div>
    );
  }

  const graduateCount = submissions.reduce(
    (sum, row) => sum + (row.company ? 1 : 0),
    0,
  );

  return (
    <main className="page-shell relative overflow-hidden">
      <Artboard nodes={nodes} view={view} />

      {/* Demo badge */}
      <div className="fixed top-3 right-3 z-10 rounded-full border border-amber-400/40 bg-amber-500/15 px-3 py-1.5 text-xs font-semibold text-amber-200 backdrop-blur sm:top-8 sm:right-8 sm:text-sm">
        STATIC DEMO
      </div>

      {/* Title overlay */}
      <div className="fixed top-3 left-3 z-10 rounded-2xl border border-slate-800/70 bg-slate-950/50 px-3 py-2 backdrop-blur sm:top-8 sm:left-8 sm:px-4 sm:py-3">
        <h1 className="mb-0 text-lg font-semibold tracking-tight text-white sm:mb-1 sm:text-4xl">
          CAIE Graduates
        </h1>
        <p className="-mt-1 text-xs italic text-slate-300/90 sm:text-sm tracking-tight">
          Formerly CESS
        </p>
        <p className="text-xs text-slate-400 sm:text-base">
          {nodes.length} {nodes.length === 1 ? "company" : "companies"} •{" "}
          {graduateCount} graduates
        </p>
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <Link
            href="/demo/browse-graduates"
            className="inline-flex rounded-full border border-slate-700/70 bg-slate-800/80 px-3 py-1.5 text-[11px] font-semibold text-slate-100 shadow-[0_10px_24px_rgba(2,8,23,0.18)] transition-all duration-300 hover:bg-slate-700"
          >
            Browse Graduates
          </Link>
          <button
            onClick={() =>
              setView((v) => (v === "bubble" ? "grid" : "bubble"))
            }
            className="inline-flex rounded-full border border-slate-700/70 bg-slate-800/80 px-3 py-1.5 text-[11px] font-semibold text-slate-100 shadow-[0_10px_24px_rgba(2,8,23,0.18)] transition-all duration-300 hover:bg-slate-700"
          >
            {view === "bubble" ? "Grid View" : "Bubble View"}
          </button>
          <Link
            href="/"
            className="inline-flex rounded-full border border-cyan-500/40 bg-cyan-500/10 px-3 py-1.5 text-[11px] font-semibold text-cyan-200 shadow-[0_10px_24px_rgba(2,8,23,0.18)] transition-all duration-300 hover:bg-cyan-500/20"
          >
            Live Board →
          </Link>
        </div>
        {exportedAt ? (
          <p className="mt-2 text-[10px] text-slate-500">
            Snapshot: {new Date(exportedAt).toLocaleString()}
          </p>
        ) : null}
      </div>

      {/* Bottom-left credit */}
      <div className="fixed bottom-3 left-3 z-10 select-none sm:bottom-5 sm:left-5">
        <SiteFooter />
      </div>
    </main>
  );
}